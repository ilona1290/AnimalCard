...
//   ];
  ...
  const columns = [
    { field: "treatmentDate", headerName: "Data zabiegu", type: "date", flex: 1, minWidth: 110, headerAlign: 'center', align: 'center', renderHeader: () => (
      ...
      { field: "name", headerName: "Nazwa zabiegu", type: "string", flex: 1, minWidth: 110, headerAlign: 'center', align: 'center', renderHeader: () => (
        <strong>
          {'Nazwa zabiegu'}
        </strong>
      )},
    ...
    { field: "treatmentDescription", headerName: "Opis", type: "string", flex: 5,
    minWidth: 400, headerAlign: 'center', align: 'center', renderHeader: () => (
        <strong>
          {'Opis'}
        </strong>
      )},
      { field: "recommendations", headerName: "Zalecenia", type: "string", flex: 3, minWidth: 200, headerAlign: 'center',align: 'center', renderHeader: () => (
        <strong>
          {'Zalecenia'}
        </strong>
      )},
...
function Treatments(){
    ...
    useEffect(() => {
        getData(`api/Pet/${petId}/Treatments`).then((result) => {
          const data = result.petTreatments.map(obj => {
            const { id, name, diagnosis, treatmentDescription, recommendations, treatmentDate, vet } = obj; // Wybierz potrzebne właściwości obiektu z pierwszej tablicy
            return { id, name, diagnosis, treatmentDescription, recommendations, treatmentDate: new Date(treatmentDate), vet }; // Utwórz nowy obiekt z wybranymi właściwościami
          })
            setRows(data);
            setLoading(false);
        })
    ...
    return(
        <div style={{paddingTop: "9em", width: "100%"}}>
          ...
          {!isLoading && SessionManager.getPets().includes(petId) && <div>
            ...
            <DataTable rows={rows} columns={columns} paddingProp="4%"/>
            </div>}
        </div>
    )
}
